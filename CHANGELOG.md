...
- **Backing plate non-manifold saddle edges**: The standard per-pixel backing and solid-core slab generator now separates vertices where retained pixels touch only diagonally. This prevents four wall triangles from sharing one vertical edge, including diagonal contacts that remain after `--trim` because each pixel has edge-connected neighbors elsewhere.
- **Trimmed model previews**: The right-hand matched-color panel generated by `--preview` now excludes pixels removed by `--trim`, so it reflects the actual 3MF model footprint while the left panel retains the pre-match source for comparison.
- **`--trim` supported one-pixel details**: One-pixel color regions such as pupils are now preserved when another opaque pixel shares an edge, while truly floating or diagonal-only singleton islands are still removed.
